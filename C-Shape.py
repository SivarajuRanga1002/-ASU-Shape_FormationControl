...
def create_C_formation(N_curve, xybound):
    """
    ...
    theta = np.linspace(-np.pi/2, np.pi/2, N_curve)  # Rotate by -90 degrees
    curve_x = xybound[0] + (xybound[1] - xybound[0]) * 0.2 + (xybound[1] - xybound[0]) * 0.4 * np.cos(theta)
    curve_y = xybound[2] + (xybound[3] - xybound[2]) * 0.5 + (xybound[3] - xybound[2]) * 0.4 * np.sin(theta)
